const translations = {
  ...
  chapter_six: {
    ...
    put_it_together_three: {
      normal: {
        ...
        bullet_two: `你收回 61,000,000 satoshis 到地址 bc1qkg62ae0wwntkzhq8td87s87c4nj5zdlj2ga8j7`,
    ...
    put_it_together_six: {
      hard: {
        ...
        bullet_two: `你收回 61,000,000 到你的地址 bc1qkg62ae0wwntkzhq8td87s87c4nj5zdlj2ga8j7`,
  ...
  chapter_eight: {
    ...
    building_blocks_five: {
      ...
      nav_title: `获取交易手续费`,
      heading: `交易数据`,
      paragraph_one: `区块中确认的交易对象是 JSON 对象，其中包含 "inputs" 和 "outputs" 数组。这些数组都是 UTXO，也称为“硬币”。硬币对象有一个 "value" 属性，以 satoshis 表示。`,
      paragraph_two: `"inputs" 数组是交易花费（销毁）的硬币，"outputs" 数组是交易创建的硬币。你可能还记得第 6 章，交易总是支付手续费以激励矿工将其包含在区块中。手续费正好是交易总输入值与总输出值之间的差额。`,
    ...
    building_blocks_six: {
      ...
      nav_title: `确定区块奖励`,
      heading: `Coinbase 交易`,
      paragraph_one: `每个区块的第一笔交易称为 coinbase，也称作“第 0 笔交易”（即 txs[0]），具有一些特殊属性。首先，它没有输入！因为它不花费任何现有硬币。其次，其输出值严格由协议定义（不管 Vanderpoole 怎么说！）。这是矿工既收取交易手续费，又生成新硬币的机制。`,
  ...
  chapter_nine: {
    ...
    opcodes_one: {
      ...
      nav_title: `比特币脚本`,
      heading: `比特币脚本`,
      paragraph_one: `我们在第 6 章提到过比特币脚本，但没有深入讲解，因为你花费的硬币是通过一个简单机制锁定的：单个签名和一个隐式脚本，通过公钥验证该签名。现在情况将变得更有趣。`,
    ...
    opcodes_two: {
      ...
      paragraph_five: `在花费比特币交易输出时，花费者所需的元素会放在交易输入的见证部分（参见第 6 章），这些元素在任何脚本处理开始前被压入栈。我们称这些元素为初始栈 (INITIAL STACK)。它们非常重要，因为它们实际上就是解锁脚本并允许花费硬币的数据！`,
    },
    opcodes_three: {
      title: `操作码`,
      nav_title: `操作码`,
      ...
      paragraph_two: `让我们演示一个示例：用数学问题 1 + 2 = ? 锁定一枚比特币。谁知道这个数学题的答案，谁就可以花费硬币。`,
    ...
    opcodes_four: {
      ...
      nav_title: `栈解决方案`,
      heading: `栈解决方案`,
      paragraph_one: `栈解决方案如下，花费交易必须包含所有这些元素。它们放在尝试花费这些硬币的输入的见证部分。让我们逐步说明：`,
      table_one: {
        headings: {
          item_one: `步骤`,
          item_two: `栈`,
    ...
    opcodes_five: {
      ...
      nav_title: `计算基础算术`,
      heading: `基础算术`,
      paragraph_one: `比特币脚本可以执行简单的数学运算。你可以使用简单的数学锁定硬币，但这样任何会算数的人都可以花费这些硬币！换句话说，不要在主网尝试。`,
    ...
    opcodes_six: {
      ...
      nav_title: `使用密码学解码`,
      heading: `简单密码学`,
      paragraph_one: `我们在前面的章节中探索过 "支付到公钥哈希"。这是在隔离见证出现前，数百万交易输出中显式编写的比特币脚本。硬币被公钥的哈希锁定。支出者必须揭示与该哈希匹配的公钥，并提供该公钥验证的签名。`,
    ...
    proposal_four: {
      ...
      paragraph_three: {
        a: `她在私下生成一个安全随机数，然后交给你哈希摘要：<span className="text-[#3DCFEF] w-fit rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">HASH256(FD3771E8)</span>。一旦她揭示这个秘密，你就可以花费收到的所有硬币，而不是提前一 `,
        b: ` 秒！`,
      },
